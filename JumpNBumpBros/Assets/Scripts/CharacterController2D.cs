...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		if (isGrounded && CrossPlatformInputManager.GetButtonDown ("Jump")) { // If grounded AND jump button pressed, then allow the player to jump
			DoJump ();
		} else if (canDoubleJump && CrossPlatformInputManager.GetButtonDown ("Jump")) {//if can jump and jump button pressed
			DoJump();
			//disable double jump until grounded
			canDoubleJump = false;
		}
		...
		//if moving up, set parameter JumpUp to true
		if (_vy > 0) 
		{
			jumpUp = true;
		} else {
		...
		//pass jumpUp to Animator
		_animator.SetBool("jumpUp", jumpUp);
	}
